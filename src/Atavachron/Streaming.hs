...
--
-- NOTE: Since we force the effects using the supplied window size,
...
evaluate n = heads . S.slidingWindow n
  where
    heads :: Stream' (Seq a) m r -> Stream' a m r
    heads str = do
        (remainder :> r) <- S.catMaybes $ mapAccum (const seqSplit) mempty str
        -- yield remainder of last window
        M.mapM_ yield remainder
        return r

    seqSplit :: Seq a -> (Seq a, Maybe a)
    seqSplit (Seq.viewl -> a Seq.:< remainder) = (remainder, Just a)
    seqSplit _ = (mempty, Nothing)
